...
public class ViewDragHelper {
     ...
     */
    private ViewDragHelper(Context context, ViewGroup forParent, Interpolator interpolator,
                           Callback cb) {
        if (forParent == null) {
        ...
        mScroller = ScrollerCompat.create(context, interpolator != null ? interpolator :
                sInterpolator);
    }
     ...
     * @return a new ViewDragHelper instance
     */
    public static ViewDragHelper create(ViewGroup forParent, Interpolator interpolator, Callback
            cb) {
     ...
     * @return a new ViewDragHelper instance
     */
    public static ViewDragHelper create(ViewGroup forParent, float sensitivity, Interpolator
            interpolator, Callback cb) {
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        ...
        mScroller.fling(mCapturedView.getLeft(), mCapturedView.getTop(), (int)
                VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId), (int)
                VelocityTrackerCompat.getYVelocity(mVelocityTracker, mActivePointerId), minLeft,
        ...
        if (mDragState == STATE_SETTLING) {
            ...
            if (dy != 0) {
                ...
                if (mHasFloatingActionButton && mIsFabAttached) {
                    ...
                    if (y <= (mSlideRange + mFabExpandedYSpace)) { // Between expanded and
                        // collapsed state
                        if (mFabMode == FloatingActionButtonLayout.FabMode.CIRCULAR_REVEAL) { //
                            // Fab follows top and gets hidden/shown with a circular reveal
                            faby = Math.round((float) (y - mFloatingActionButton
                                    .getMeasuredHeight() / 2));
                        ...
                        else if (mFabMode == FloatingActionButtonLayout.FabMode.FADE) { // Fab
                            // follows top and fades away
                            faby = Math.round((float) (y - mFloatingActionButton
                                    .getMeasuredHeight() / 2));
                            ...
                            if (!mHasAnchor) {
                                faby = Math.round(mFabRatio * (y - mFabExpandedYSpace)) +
                                        mFabExpandedY;
                            }
                            else {
                                if (y <= mAnchorY) {
                        ...
                        faby = Math.round((((float) (y - (mSlideRange + mFabExpandedYSpace))) / (
                                (float) mPanelHeight)) * mFabHideDeltaY) + mFabCollapsedY;
                    }
        ...
        if (v instanceof ViewGroup) {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (x + scrollX >= child.getLeft() && x + scrollX < child.getRight() &&
                        ...
                        canScroll(child, true, dx, dy, x + scrollX - child.getLeft(), y + scrollY
                                - child.getTop())) {
                    return true;
                }
            }
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                final int pointerCount = MotionEventCompat.getPointerCount(ev);
                for (int i = 0; i < pointerCount && mInitialMotionX != null && mInitialMotionY !=
                        null; i++) {
                    final int pointerId = MotionEventCompat.getPointerId(ev, i);
                    if (pointerId >= mInitialMotionX.length || pointerId >= mInitialMotionY
                            .length) {
                        continue;
                    }
                    final float x = MotionEventCompat.getX(ev, i);
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING) {
                    ...
                    for (int i = 0; i < pointerCount; i++) {
                        ...
                        final View toCapture = findTopChildUnder((int)
                                mInitialMotionX[pointerId], (int) mInitialMotionY[pointerId]);
    ...
    private void releaseViewForPointerUp() {
        ...
        final float xvel = clampMag(VelocityTrackerCompat.getXVelocity(mVelocityTracker,
                mActivePointerId), mMinVelocity, mMaxVelocity);
        final float yvel = clampMag(VelocityTrackerCompat.getYVelocity(mVelocityTracker,
                mActivePointerId), mMinVelocity, mMaxVelocity);
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        ...
        if (dy != 0) {
            ...
            if (mHasFloatingActionButton && mIsFabAttached) {
                ...
                int faboldTop = mFloatingActionButton.getTop();
                if (clampedY <= (mSlideRange + mFabExpandedYSpace)) { // Between expanded and
                    // collapsed state
                    if (mFabMode == FloatingActionButtonLayout.FabMode.CIRCULAR_REVEAL) { // Fab
                        // follows top and gets hidden/shown with a circular reveal
                        fabclampedY = Math.round((float) (clampedY - mFloatingActionButton
                                .getMeasuredHeight() / 2));
                        ...
                        if (!mHasAnchor) {
                            fabclampedY = Math.round(mFabRatio * (clampedY - mFabExpandedYSpace))
                                    + mFabExpandedY;
                        }
                        else {
                            if (clampedY <= mAnchorY) {
                                fabclampedY = Math.round(mFabRatio * (clampedY -
                                        mFabExpandedYSpace)) + mFabExpandedY;
                            }
                            else {
                                fabclampedY = Math.round((float) (clampedY -
                                        mFloatingActionButton.getMeasuredHeight() / 2));
                            }
                        }
                    }
                }
                else { // Between collapsed and hidden state
                    fabclampedY = Math.round((((float) (clampedY - (mSlideRange +
                            mFabExpandedYSpace))) / ((float) mPanelHeight)) * mFabHideDeltaY) +
                            mFabCollapsedY;
                }
    ...
    protected void setFadeOutAnimation() {
        mFadeOutAnimation = AnimationUtils.loadAnimation(mFloatingActionButton.getContext(), R
                .anim.fade_scale_out);
        mFadeOutAnimation.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                setFadeOutAnimation();
                if (mFabVisibility == View.VISIBLE)
    ...
    protected void setFadeInAnimation() {
        mFadeInAnimation = AnimationUtils.loadAnimation(mFloatingActionButton.getContext(), R
                .anim.fade_scale_in);
        mFadeInAnimation.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                setFadeInAnimation();
                Log.i("Android Panel", "Fab should be visible? " + (mFabVisibility == View
                        .VISIBLE));
                if (mFabVisibility == View.VISIBLE)
    ...
    protected void hideFloatingActionButton() {
        if (mFadeOutAnimation != null && !mFadeOutAnimation.hasStarted() && mFloatingActionButton
    ...
    protected void showFloatingActionButton() {
        if (mFadeInAnimation != null && !mFadeInAnimation.hasStarted() && mFloatingActionButton
